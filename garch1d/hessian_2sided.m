...
function [VCV] = hessian_2sided(x,data,rf)
...
estim_flag = 0;
[fx,like] = log_likelihood_chris(x,data,estim_flag,rf);
...
likem=zeros(t,n);
for i=1:n
    [gp(i),likep(:,i)] = log_likelihood_chris(x+ee(:,i),data,estim_flag,rf);
    [gm(i),likem(:,i)] = log_likelihood_chris(x-ee(:,i),data,estim_flag,rf);
end
...
for i=1:n
    for j=i:n
        Hp(i,j) = log_likelihood_chris(x+ee(:,i)+ee(:,j),data,estim_flag,rf);
        Hp(j,i) = Hp(i,j); %symmetric
        Hm(i,j) = log_likelihood_chris(x-ee(:,i)-ee(:,j),data,estim_flag,rf);
        Hm(j,i) = Hm(i,j);
    end
end
%Compute the hessian
