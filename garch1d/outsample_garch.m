...
for i = ninsample : 1 : n-1
    ...
    [param, loglik , exitflag, fo] = fminunc_garch(insample,rf,prevparam);
    if exitflag == 0 || fo > 0.1
        prevparam = [];
        [param, loglik , exitflag, fo] = fminunc_garch(insample,prevparam);
    else 
        prevparam = param;
    end
    simuls = vecsimulate(dayahead,param,rf,nsim); %usavo 2 prima meglio 1?
